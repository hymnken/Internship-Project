<template>
  <div style="width: 100%">
    ...
    <van-form @submit="onSubmit" style="margin-top: 10px">
      <van-field v-if="checked" name="层" label="层" placeholder="请填写层的名称" v-model="layer.layerNumber" :rules="[
        { pattern: pattern, message: '格式错误(仅支持中英文和数字)' },
      ]" />
      <van-field v-if="!checked" readonly clickable name="picker" :value="selValue" label="选择层名" placeholder="点击选择层名"
        @click="loadPicker" />
