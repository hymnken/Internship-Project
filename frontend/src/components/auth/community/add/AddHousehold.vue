<template>
  <div style="width:100%">
    ...
    <van-form @submit="onSubmit" style="margin-top: 10px">
      <van-field v-if="checked" name="房间" label="房间" placeholder="请填写房间的名称" v-model="household.householdNumber" :rules="[
        { pattern: pattern, message: '格式错误(仅支持中英文和数字)' },
      ]" />
      <van-field v-if="!checked" readonly clickable name="picker" :value="selValue" label="选择房间名" placeholder="点击选择房间名"
        @click="loadPicker" />
