<template>
  <div style="width: 100%">
    ...
    <van-form @submit="onSubmit" style="margin-top: 10px">
      <van-field v-if="checked" name="期" label="期" placeholder="请填写期名" v-model="period.periodNumber" :rules="[
        { pattern: pattern, message: '期名格式错误(支持中英文和数字)' },
      ]" />
      <van-field v-if="!checked" readonly clickable name="picker" :value="selValue" label="选择期名" placeholder="点击选择期名"
        @click="loadPicker" />
