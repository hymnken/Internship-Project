<template>
  <div style="width: 100%">
    ...
    <van-form @submit="onSubmit" style="margin-top: 10px">
      <van-field v-if="checked" name="单元" label="单元" placeholder="请填写单元的名称" v-model="unit.unitNumber" :rules="[
        { pattern: pattern, message: '格式错误(仅支持中英文和数字)' },
      ]" />

      <van-field v-if="!checked" readonly clickable name="picker" :value="selValue" label="选择单元名" placeholder="点击选择单元名"
        @click="loadPicker" />
