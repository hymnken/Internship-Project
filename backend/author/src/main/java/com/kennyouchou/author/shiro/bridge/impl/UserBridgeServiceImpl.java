package com.kennyouchou.author.shiro.bridge.impl;
...
import com.kennyouchou.author.shiro.bridge.UserBridgeService;
import com.kennyouchou.author.shiro.cache.SimpleMapCache;
...
public class UserBridgeServiceImpl implements UserBridgeService {
    ...
    public List<String> findResourceIds(String userId) {
        ...
        if (Objects.nonNull(cache)){
            // 缓存存在
            List<com.kennyouchou.author.entity.Resource> resources = (List< com.kennyouchou.author.entity.Resource>) cache.get(key);
            return  resources.stream().map(com.kennyouchou.author.entity.Resource::getId).collect(Collectors.toList());
        }
        // 缓存不存在
        List<com.kennyouchou.author.entity.Resource> resourceList = resourceService.getResourceByUserId(userId);
        if (! CollectionUtils.isEmpty(resourceList)) {
            // 创建缓存
            Map<Object, Object> map = new HashMap<>();
            map.put(key, resourceList);
            simpleMapCacheService.createCache(key, new SimpleMapCache(key, map));
            return resourceList.stream().map(com.kennyouchou.author.entity.Resource::getId).collect(Collectors.toList());
        }
        return null;
    }
    ...
    public List<String> findResourcesList(String key,String userId){
        List<com.kennyouchou.author.entity.Resource> resources = new ArrayList<>();
        //获得缓存
        Cache<Object, Object> cache = simpleMapCacheService.getCache(key);
        //缓存存在
        if (!ObjectUtil.isEmpty(cache)){
            resources = (List<com.kennyouchou.author.entity.Resource>) cache.get(key);
        }else {
            //缓存不存在
        ...
        for (com.kennyouchou.author.entity.Resource resource : resources) {
