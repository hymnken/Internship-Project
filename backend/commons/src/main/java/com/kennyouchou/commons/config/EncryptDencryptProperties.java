package com.kennyouchou.commons.config;

import lombok.Data;
import org.springframework.boot.context.properties.ConfigurationProperties;
import org.springframework.stereotype.Component;

/**
 * <p>
 * 加密解密配置类
 * </p>
 *
 * @author kennyouchou
 * @since 2022-10-23 18:55:56
 */
@Component
@ConfigurationProperties(
        prefix = "kennyouchou.enc-denc"
)
@Data
public class EncryptDencryptProperties {
    /**
     * 开启加密解密
     * @since 2022/10/23 18:58
     **/
    private Boolean encAndDenc = true;

    /**
     * 加密解密方式
     * @since 2022/10/23 18:58
     **/
    private String encryptType = "RSA";

    /**
     * <p>
     *     RSA加密方式，非对称式加密
     * </p>
     * @author kennyouchou
     * @since 2022/10/30 10:13
     **/
    @Data
    @Component
    public class RsaKey {
        /**
         * 私钥用于解密
         * @since 2022/10/30 13:40
         **/
        private String parivateKeyByBase64 = "";
        /**
         * 公钥用于加密
         * @since 2022/10/30 13:40
         **/
        private String publicKeyByBase64 = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDSdnWiMfG0FOs2kel546ftXlFXUycgsEPuuHEHG/y9avT7WLkmRMxw1BgcWEgWMd9x1jMFPCxxWwd2oKr+v4nXMif+5nvPWpkNcyRzQ5aUIvUpANtdthlkRxSG6v9kxMaH5z+PmBNGMu9/N2SBPk4YTCuELJDH9E9gvuGkVdmpRQIDAQAB";

    }

    /**
     * <p>
     *     AES加密方式，对称式加密
     * </p>
     * @author kennyouchou
     * @since 2022/10/30 13:39
     **/
    @Component
    @Data
    public class AesKey{
        /**
         * 加密解密
         * @since 2022/10/30 13:41
         **/
        private String key = "dsrHDJUiRxKIMe/YiEhcWQ==";
    }

}
